...
def train(buffer, pnet, pnet_target, vnet, vnet_target, optim_p, optim_v, train_p = False):
    ...
    with torch.no_grad():
        ...
        next_action += 0.1*torch.randn_like(next_action)
        ...
        target = reward + GAMMA * (1 - done) * torch.min(next_qval1, next_qval2)
    ...
    if not train_p: return None

    for param in vnet.parameters():
        param.requires_grad = False

    qval = vnet.dqn1(state, pnet(state))
...
for nstep in range(NSTEPS):
    ...
    if len(buffer) >= BATCH_SIZE:
        loss = train(buffer, pnet, pnet_target, 
                     vnet, vnet_target, 
                     optim_p, optim_v, 
                     (nstep+1)%NTRAIN == 0)
