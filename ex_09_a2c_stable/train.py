...
class EnvWrapper(object):
    ...
    def step(self, action):
        ...
        # return np.stack(self.frame, 0), reward, done, {}
...
#for nstep in range(NSTEPS):
    ...
    if done or len(buffer) == BATCH_SIZE:
        ...
        loss = 0.9*loss + 0.1*train(buffer, next_value, pnet, vnet, optimizer)
        # break
        buffer.reset()
    ...
    if (nstep + 1) % 10000 == 0:
        ...
        plt.plot(all_losses)
        plt.subplot(223)
        plt.plot(moving_average(all_rewards, 10))
        plt.subplot(224)
        plt.plot(all_values)
        plt.show()
